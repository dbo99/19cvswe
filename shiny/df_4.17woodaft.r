...
#
  ...
  df$rivgroup <- nwscode_to_rivgroup$rivgroup[match(df$nwscode,nwscode_to_rivgroup$nwscode)] 
  ...
  df <- df %>% mutate(nws_basin_code = as.factor(paste0(basin," (", nwscode, ")")), nwscode = as.factor(nwscode), basin = as.factor(basin),
              param = as.factor(param), basin_zone = as.factor(basin_zone), wy = as.factor(water_year(date),
              rivgroup = as.factor(rivgroup))) %>% select(-cumdoy, -yday)
...
df$paramnam <- factor(df$paramnam, levels=c('areal extent of coverage (sca) [%]', 'average snowpack temperature (se) [F]', 'blowing snow sublimation (sb) [in]', 'depth (sd) [in]', 
                                     'melt (sm) [in]', 'surface sublimation (su) [in]', 'water equivalent (swe) [in]'))
