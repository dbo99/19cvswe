...
for (i in 1:length(dates)) { 
  ...
  for (j in 1:length(params)) {
    ...
    num = as.integer(url8)
    if(num %% 2 == 0) {Sys.sleep(0.5) } #if even (no remainder),give  break
    if(num %% 9 == 0) {Sys.sleep(1.5) }# if 9,18,27th day of month, give more break (all to not overload server)
    ...
    df <- rbind(df,dftemp_i) %>% unique() %>% na.omit()
    
  }  
}

df1 <- add_timeclasses(df)

write_csv(df, "wy2017_allp_thruApr7.csv")