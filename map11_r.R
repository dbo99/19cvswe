...
setwd("~/R/proj/gis")
source("nohrsc_swe_scrape.r")
cvsnowbasins <- read_csv("CNRFC_melt_basinIDs.csv")
swevol <- read_csv("2019032712_CNRFC_SnowVolume_csv_Export.csv")
swevol <- swevol %>% mutate(row = row_number()) %>%
           filter(row != 1)  %>% select(-row) %>% gather("basin_8","swe_vol", 2:ncol(swevol)) %>%
           mutate(GMT = ymd_hms(GMT), GMT_d = date(GMT) , swe_vol = as.double(swe_vol))  
           
swevol <- swevol %>% mutate(swe_kaf = round(swe_vol/1000,0) ) %>%
...
#htmlwidgets::saveWidget(m, "wtf.html", selfcontained = T)
